...
from fastprop.model import train_and_test, fastprop 
...
NUM_REPLICATES = 4
SPLIT_TYPE = "random"  # solute, random
RANDOM_SEED = 3511  # the final frontier
TRAINING_FPATH = Path("/your/directory")  # decriptors
...
def logS_within_0_7_percentage(
    truth: torch.Tensor,
    prediction: torch.Tensor,
    ignored: None,
    multitask: bool = False,
):
    return (truth - prediction).abs().less_equal(0.7).count_nonzero() / prediction.size(
        dim=0
    )

def logS_within_1_0_percentage(
    truth: torch.Tensor,
    prediction: torch.Tensor,
    ignored: None,
    multitask: bool = False,
):
    return (truth - prediction).abs().less_equal(1.0).count_nonzero() / prediction.size(
        dim=0
    )
...
def train_ensemble(*, data=None, remove_output=False, training_percent=None, **model_kwargs):
    ...
    _output_dir = Path(f"output_weight1/fastprop_{int(datetime.now(timezone.utc).timestamp())}")
    ...
    if data is None:
        TRAINING_FPATH = "/your/directory"
        df = pd.read_csv(_data_dir / TRAINING_FPATH, index_col=0)
        metadata_df_og = df[["solute_smiles", "solvent_smiles", "source"]]
        solubilities_og = torch.tensor(pd.to_numeric(df["logS"], errors="coerce").fillna(0).to_numpy(), dtype=torch.float32).unsqueeze(-1)
...
if __name__ == "__main__":
    ...
    for training_count in (100, 200000): 
        training_percent = training_count / 200000
        train_ensemble(
