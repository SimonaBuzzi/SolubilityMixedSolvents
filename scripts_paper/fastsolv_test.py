...
from classes import fastpropSolubility, SolubilityDataset
...
def test_ensemble(checkpoint_dir: Path):
    ...
    for holdout_fpath in (
        Path("/your/path/to/descriptors/descriptors.csv"),
        ...
        mask = ~np.isnan(out["logS_true"]) & ~np.isnan(out["logS_pred"])
        y_true = out["logS_true"][mask]
        y_pred = out["logS_pred"][mask]
        ...
        wn_07 = np.count_nonzero(np.abs(y_true - y_pred) <= 0.7) / len(y_pred)
        wn_1 = np.count_nonzero(np.abs(y_true - y_pred) <= 1.0) / len(y_pred)
        ...
        parity_plot(
            ...
            y_pred, 
            holdout_fpath.stem, 
            _output_dir / f"{holdout_fpath.stem}_parity.png", 
            stat_str
        )
...
if __name__ == "__main__":
    ...
    fastpropMixresults = []
    for training_count in [226483]: 
        # predictions = test_ensemble(Path(f"scaffold_random_correct/fastprop_{training_count}/checkpoints"))
        predictions = test_ensemble(Path(f"output_weight1/fastprop_{training_count}/checkpoints/"))
        fastpropMixresults.append([predictions])
    print(f"{fastpropMixresults=}")
