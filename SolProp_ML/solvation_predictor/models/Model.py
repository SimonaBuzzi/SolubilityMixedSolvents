...
class Model(nn.Module):
    ...
    #         # update_vecs = torch.zeros(1, update_vec_size, device=self.inp.device)  # keep [1, hidden_dim]

    #         # for i in range(1, self.max_num_mols):
    #         #     molefrac_col = molefracs[:, i - 1].unsqueeze(1)       # [1, 1]
    #         #     update_vec = mol_encodings[i] * molefrac_col          # [1, hidden_dim]
    #         #     update_vecs = update_vecs + update_vec                # [1, hidden_dim]

    #         # input = torch.cat([mol_encodings[0], update_vecs], dim=1)  # [1, 2*hidden_dim]
    #         # return input

    #     update_vec_size = mol_encodings[0].size(1)
    #     update_vecs = torch.zeros(update_vec_size, device=self.inp.device)
    #     vec = update_vecs.unsqueeze(0)        # [1, hidden_dim]
    ...
    def solute_forward(self, datapoints, tensors, molefracs):
        """
        Forward pass for the MolPool architecture for infinite-dilution mixtures. Corresponding to solute set to True in
        the input arguments.
        """
        mol_encodings, atoms_vecs = self.mpn_1(tensors[0])
        mol_encodings = mol_encodings[None, :]
